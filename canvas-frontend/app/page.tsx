...
export default function CanvasPage() {
  ...
  if (!user) {
    return (
      <main className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-[#f6f8fb] px-4 py-6 text-slate-900">
        ...
        <div className="relative w-full max-w-[1160px] overflow-hidden rounded-lg border border-white/80 bg-white/90 shadow-[0_24px_90px_rgba(15,23,42,0.12)] backdrop-blur-xl">
          <div className="grid gap-0 md:grid-cols-[1.15fr_0.85fr]">
            <div className="flex flex-col justify-between border-b border-slate-100 px-8 py-8 md:border-b-0 md:border-r md:px-10 md:py-10">
              <div>
                <div className="inline-flex items-center gap-2 rounded-md border border-cyan-100 bg-cyan-50 px-3 py-1 text-[11px] font-black uppercase tracking-[0.18em] text-cyan-700">
                  <Sparkles className="h-3.5 w-3.5" />
                  AgentFlow Studio
                </div>
                <h1 className="mt-5 max-w-xl text-4xl font-black text-slate-950 md:text-5xl">Enterprise AI workflow orchestration, built for real operations.</h1>
                ...
                <div className="mt-8 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
                  <div className="mb-4 flex items-center justify-between border-b border-slate-100 pb-3">
                    <div>
                      <div className="text-xs font-black uppercase tracking-[0.12em] text-slate-400">Live workflow</div>
                      <div className="mt-1 text-sm font-black text-slate-950">Revenue Ops Agent</div>
                    </div>
                    <span className="rounded-md border border-emerald-200 bg-emerald-50 px-2 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-emerald-700">Ready</span>
                  </div>
            ...
            <div className="px-8 py-8 md:px-10 md:py-10">
              <div className="flex items-center gap-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-lg border border-cyan-100 bg-cyan-50 text-cyan-700"><LockKeyhole className="h-5 w-5" /></div>
                <div>
              ...
              <div className="mt-6 grid grid-cols-2 gap-1 rounded-lg bg-slate-100 p-1 text-sm font-black">
                <button onClick={() => setAuthMode('login')} className={`rounded-md py-2.5 ${authMode === 'login' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-400'}`}>Login</button>
                <button onClick={() => setAuthMode('register')} className={`rounded-md py-2.5 ${authMode === 'register' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-400'}`}>Register</button>
              </div>
              <div className="mt-4 space-y-3">
                <input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email" className="h-12 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100" />
                <input value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password" type="password" className="h-12 w-full rounded-lg border border-slate-200 bg-white px-4 text-sm font-semibold outline-none focus:border-cyan-400 focus:ring-4 focus:ring-cyan-100" />
                <button onClick={handleAuth} disabled={loading} className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-lg bg-slate-950 text-sm font-black text-white transition hover:bg-cyan-700 disabled:bg-slate-300">
  ...
  return (
    <main className="relative flex min-h-dvh w-screen flex-col bg-[#f6f8fb] text-slate-900 antialiased md:h-dvh md:flex-row md:overflow-hidden">
      <aside className="z-20 flex max-h-[72dvh] shrink-0 flex-col border-b border-slate-200/80 bg-white/92 shadow-[0_18px_44px_rgba(15,23,42,0.08)] backdrop-blur-2xl md:h-full md:max-h-none md:w-[430px] md:border-b-0 md:border-r">
        <div className="border-b border-slate-100 px-4 py-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-cyan-100 bg-cyan-50 text-cyan-600 shadow-sm"><Sparkles className="h-5 w-5" /></div>
        ...
        <div className="sidebar-scroll min-h-0 flex-1 overflow-y-auto px-4 py-4">
          <section className="rounded-lg border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-4 shadow-[0_12px_26px_rgba(15,23,42,0.04)]">
            <div className="flex items-end justify-between">
              ...
              <div className="rounded-md border border-cyan-100 bg-cyan-50 px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-cyan-700">Connected</div>
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2">
              {[
                ...
                <div key={metric.label} className={`rounded-lg border bg-gradient-to-br p-3 shadow-sm ${kpiTone(index)}`}>
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-3 shadow-[0_12px_26px_rgba(15,23,42,0.04)]">
            <div className="mb-2 flex items-center justify-between">
              <div className="text-xs font-black uppercase tracking-[0.12em] text-slate-500">Projects</div>
              <span className="rounded-md border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">Workspace</span>
            </div>
            <div className="grid grid-cols-[1fr_auto] gap-2">
              <input value={newProjectName} onChange={(event) => setNewProjectName(event.target.value)} placeholder="New project" className="h-10 rounded-lg border border-slate-200 px-3 text-xs font-semibold outline-none focus:border-cyan-400" />
              <button onClick={createProject} className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-950 text-white" aria-label="Create project"><Plus className="h-4 w-4" /></button>
            </div>
            <div className="mt-3 grid gap-2">
              {projects.map((project) => (
                <button key={project.id} onClick={() => selectProject(project.id)} className={`rounded-lg border px-3 py-2.5 text-left text-xs font-bold transition ${projectId === project.id ? 'border-cyan-200 bg-cyan-50 text-cyan-800' : 'border-slate-200 text-slate-600 hover:border-cyan-200 hover:bg-slate-50'}`}>{project.name}</button>
              ))}
              {projects.length === 0 && <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-400">Create a project to start a workspace.</div>}
            </div>
          </section>
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-3 shadow-[0_12px_26px_rgba(15,23,42,0.04)]">
            ...
            <button type="button" onClick={generateCanvas} disabled={loading || !prompt.trim() || !projectId} className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-4 text-sm font-black text-white shadow-[0_14px_28px_rgba(15,23,42,0.2)] transition hover:bg-cyan-700 disabled:cursor-not-allowed disabled:bg-slate-300 disabled:shadow-none">
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-3 shadow-[0_12px_26px_rgba(15,23,42,0.04)]">
            ...
            <div className="grid gap-2">
              {templates.map((template) => (
                <button key={template.id} onClick={() => applyTemplate(template)} className="rounded-lg border border-slate-200 p-3 text-left transition hover:border-cyan-200 hover:bg-cyan-50">
              ...
              {templates.length === 0 && <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-400">Templates load after sign-in completes.</div>}
            </div>
          </section>
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-3 shadow-[0_12px_26px_rgba(15,23,42,0.04)]">
            ...
            <div className="grid gap-2">
              {canvases.map((canvas) => (
                <button key={canvas.id} onClick={() => openCanvas(canvas)} className={`rounded-lg border p-3 text-left transition ${activeCanvas?.id === canvas.id ? 'border-cyan-200 bg-cyan-50' : 'border-slate-200 hover:border-cyan-200 hover:bg-slate-50'}`}>
              ...
              {canvases.length === 0 && <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-400">Saved workflows will appear here.</div>}
            </div>
          </section>
          ...
          <section className="mt-4 rounded-lg border border-slate-200 bg-white p-3 shadow-[0_12px_26px_rgba(15,23,42,0.04)]">
            ...
            <div className="grid max-h-64 gap-2 overflow-y-auto pr-1">
              ...
              {connectors.length === 0 && <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-400">Connector catalog is waiting for backend data.</div>}
            </div>
          </section>
        </div>
      ...
      <section className="relative min-h-[560px] flex-1 overflow-hidden md:min-h-0">
        <div className="absolute inset-0 bg-[linear-gradient(135deg,#f8fafc_0%,#eef7f9_48%,#f8fafc_100%)]" />
        <div className="absolute inset-0 opacity-45 [background-image:radial-gradient(#94a3b8_1px,transparent_1px)] [background-size:28px_28px]" />

        <div className="absolute left-4 right-4 top-4 z-10 rounded-lg border border-white/80 bg-white/90 px-4 py-3 shadow-[0_16px_44px_rgba(15,23,42,0.08)] backdrop-blur-xl">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="inline-flex items-center gap-1 rounded-md border border-cyan-100 bg-cyan-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-700">
                ...
                <span className="rounded-md border border-slate-200 bg-white px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">{user.email}</span>
              </div>
              <div className="mt-2 flex items-center gap-3">
                <h2 className="text-lg font-black text-slate-950">{activeCanvas?.title || 'Canvas workspace'}</h2>
                <span className={`rounded-md border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${latestRun ? statusClass(latestRun.status) : 'border-slate-200 bg-slate-50 text-slate-500'}`}>{latestRun ? latestRun.status : 'idle'}</span>
              </div>
              <p className="mt-1 max-w-3xl text-sm text-slate-500">{activeCanvas?.summary || summary}</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button onClick={() => runCanvas(false)} disabled={!activeCanvas || loading} className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs font-black text-emerald-700 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400"><Play className="h-3.5 w-3.5" /> Run</button>
              <button onClick={() => runCanvas(true)} disabled={!activeCanvas || loading} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 disabled:cursor-not-allowed disabled:text-slate-400"><Clock3 className="h-3.5 w-3.5" /> Dry Run</button>
              {waitingStep && <button onClick={() => approveWaitingStep(true)} disabled={loading} className="inline-flex items-center gap-1 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-black text-amber-700"><ShieldCheck className="h-3.5 w-3.5" /> Approve</button>}
              <button onClick={exportPng} className="inline-flex items-center gap-1 rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-black text-cyan-700"><Download className="h-3.5 w-3.5" /> PNG</button>
              <button onClick={exportPdf} className="inline-flex items-center gap-1 rounded-lg border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-black text-cyan-700"><FileText className="h-3.5 w-3.5" /> PDF</button>
            </div>
          </div>
        </div>
        ...
        {hasCanvas ? (
          <ReactFlow nodes={nodes} edges={edges} nodeTypes={nodeTypes} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} onNodeClick={(_, node) => setSelectedNodeDetails(node.data.rawDetails)} onPaneClick={() => setSelectedNodeDetails(null)} fitView fitViewOptions={{ padding: 0.2 }} minZoom={0.25} maxZoom={1.5} proOptions={{ hideAttribution: true }}>
            ...
            <Controls className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg [&_button]:border-slate-100 [&_button]:bg-white [&_svg]:fill-slate-500" />
            <Panel position="top-center" className="!m-4">
              <div className="flex flex-wrap items-center gap-2 rounded-lg border border-white/80 bg-white/90 px-3 py-2 text-xs font-black text-slate-600 shadow-[0_16px_44px_rgba(15,23,42,0.08)] backdrop-blur-xl">
                <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{nodes.length} nodes</span>
                <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{edges.length} paths</span>
                <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{runs.length} runs</span>
                <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 bg-slate-50 px-2 py-1">{connectors.length} connectors</span>
              </div>
            </Panel>
          </ReactFlow>
        ) : (
          <div className="relative z-10 flex h-full items-center justify-center px-6 pt-32 md:pt-20">
            <div className="max-w-2xl rounded-lg border border-white/80 bg-white/90 p-8 text-center shadow-[0_24px_70px_rgba(15,23,42,0.08)] backdrop-blur-xl md:p-10">
              <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-lg border border-cyan-100 bg-cyan-50 text-cyan-700"><Layers3 className="h-7 w-7" /></div>
        ...
        <aside className="absolute right-4 top-28 z-10 hidden w-96 rounded-lg border border-white/80 bg-white/92 p-3 shadow-[0_16px_44px_rgba(15,23,42,0.08)] backdrop-blur-xl xl:block">
